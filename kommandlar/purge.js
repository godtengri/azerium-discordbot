...
if(isNaN(args[0])) return message.channel.send('Xəta! Ya heç bir rəqəm yazmadınız ya da yazdığınız rəqəm deyil! Bu hələ də davam edərsə dəstək serveri: https://azerium.team/');
if(args[0] > 100) {
...
if(sayı <= 0) return message.channel.send("Mesajlar silinə bilmədi, 2 həftədən çox yazılmış mesajları silə bilmirəm.");
...
message.channel.send(`${await data.fetch(`sizee.${message.id}`)} mesaj(lar) silindi\n\n${nbr.join('\n')}`)
...
exports.help = {
  name: 'purge',
  description: 'Yazdığınız miqdarda mesaj silər'
};
